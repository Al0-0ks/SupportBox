...
function Get-ActiveNetworkConfig {
    ...
    $ConfigAvecPasserelle = $Configs | Where-Object {
        $_.DefaultIPGateway -and (Get-FirstIPv4 $_.IPAddress)
    } | Select-Object -First 1

    if ($ConfigAvecPasserelle) {
        return $ConfigAvecPasserelle
    }
...
function Get-InfoInternet {
    ...
    $ConfigReseauActive = Get-ActiveNetworkConfig
    # Sécurité : récupère la première passerelle IPv4 si elle existe, sinon attribue une valeur nulle ($null)
    $GW = if ($ConfigReseauActive) { Get-FirstIPv4 $ConfigReseauActive.DefaultIPGateway } else { $null }
    $PasserelleOK = $false
    ...
    if ($InternetOK) {
        ...
        if (-not $DnsOK) {
            ...
            Write-Host "ACTION     : Le DNS fonctionne, mais le flux HTTP/HTTPS est bloqué (vérifiez les identifiants proxy)." -ForegroundColor Yellow
        }
    }
...
function Get-InfoLecteursReseau {
    ...
    $ListeLecteurs | ForEach-Object {
        ...
        if ($Serveur -and (Test-Connection -ComputerName $Serveur -Count 1 -Quiet -ErrorAction SilentlyContinue)) {
            Write-Host "  | État du lecteur   : ACCESSIBLE (Serveur en ligne)" -ForegroundColor Green
        } else {
            Write-Host "  | État du lecteur   : HORS LIGNE ou INACCESSIBLE" -ForegroundColor Red
        }
...
function Install-FichierConfig {
    ...
    $ConfigReseauActive = Get-ActiveNetworkConfig
    $IP = if ($ConfigReseauActive) { Get-FirstIPv4 $ConfigReseauActive.IPAddress } else { $null }
    $GW = if ($ConfigReseauActive) { Get-FirstIPv4 $ConfigReseauActive.DefaultIPGateway } else { $null }
    
    $StatutGW = "ÉCHEC"
    if ($GW -and (Test-Connection -ComputerName $GW -Count 1 -Quiet -ErrorAction SilentlyContinue)) { $StatutGW = "OK" }
    ...
    if ($Lecteurs) {
        foreach ($Lecteur in $Lecteurs) {
            ...
            if ($Serveur) {
                if (Test-Connection -ComputerName $Serveur -Count 1 -Quiet -ErrorAction SilentlyContinue) {
...
function Get-TicketSupport {
    ...
    $ConstatDNS = "La résolution DNS est non testée."
